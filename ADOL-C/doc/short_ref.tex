...
...
double x[n], y[m];
auto I=unitMatrix(m);                   // allocation of identity matrix
auto J=Matrix<double>(m,n);             // allocation of Jacobian matrix
...
initialize(x);                              // setting up the argument x
...
zos_forward(ftag,m,n,1,x,y);                // computing the Jacobian by
fos_reverse(ftag,m,n,m,I.data(),J.data());  // reverse mode of AD
...
\end{verbatim}
\end{minipage}
\end{center}
...
...
double x[n];
auto I=unitMatrix(n);                // allocation of identity matrix
auto X=Matrix<double>(n,5);          // allocation of matrix X
auto A=Tensor<double>(n,n,4);        // allocation of tensors A and B
auto B=Tensor<double>(n,n,4);
...
initialize(X);                       // setting up the argument x_0
...
forodec(ftag,n,1.0,0,4,X.data());    // compute x_1,...,x_4
hov_reverse(ftag,n,n,3,n,I.data(),
            A.data(),NULL);          // compute A_0,...,A_3
accodec(ftag,n,1.0,3,A.data(),
        B.data(),NULL);              // accumulate B_0,...,B_3
...
\end{verbatim}
\end{minipage}
%
